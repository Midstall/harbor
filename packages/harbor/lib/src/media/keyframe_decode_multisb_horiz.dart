...
/// Multi-superblock HORIZONTAL keyframe intra decode: coded bytes -> two 8x8
/// luma superblocks side by side (SB0 at sb_col 0, SB1 to its right, both in
/// tile row 0), where SB1's intra left column is SB0's reconstructed right
/// column.
///
/// The horizontal counterpart of [HarborKeyframeDecodeMultiSbVert]:
///  - [HarborKeyframeModeWalk] with `multiSb` (continuous entropy walk): SB0 is
///    a fresh `start` (cont/above_open/cont_left/left_open all 0). SB1 continues
///    the same window (cont = 1, adapted CDFs + propagated above-context) at the
///    tile-top (above_open = 0), preserving the left-context arrays
///    (cont_left = 1, AV1 keeps left context across SBs in a row) with a decoded
///    SB to its left (left_open = 1).
///  - [HarborIntraReconWalk] with `tiled` (tile-relative availability + external
///    neighbour inputs). SB0 recons at the tile-origin SB (sb_c = 0), SB1 at
///    sb_c = 2 (2 mi units right) with `ext_left` = SB0's reconstructed right
///    column (frame0 col 7). At tile row 0 both SBs are tile-top (haveA false),
///    so ext_above / ext_corner are unused and driven 0.
///
...
/// one continuous window, recons SB0, then SB1 (left = SB0 right column), then
/// asserts `done`.
class HarborKeyframeDecodeMultiSbHoriz extends BridgeModule {
  ...
  HarborKeyframeDecodeMultiSbHoriz({
    this.maxBytes = 48,
    this.qband = 0,
    String? name,
       ...
       super(
         'HarborKeyframeDecodeMultiSbHoriz',
         name: name ?? 'keyframe_decode_multisb_horiz',
       ) {
    ...
    // two latched leaf-arrays (one per SB)
    List<Logic> mkCap() => [
    ...
    // SB0's reconstructed RIGHT column (frame col 7), the 8 pixels, packed as
    // the ext_left input for SB1: ext_left[i*8 +: 8] = frame0 pixel (row i,
    // col 7).
    ...
    // recon SB1 (sb_c = 2 mi units right, left = SB0 right column)
    ...
    // SB1 sits one 8x8 (2 mi units) right of the tile left, same tile row 0.
    recon1.input('sb_r').srcConnection! <= Const(0, width: miAbsW);
    recon1.input('sb_c').srcConnection! <= Const(2, width: miAbsW);
    recon1.input('tile_top_mi').srcConnection! <= Const(0, width: miAbsW);
    recon1.input('tile_left_mi').srcConnection! <= Const(0, width: miAbsW);
    // LEFT column = SB0's reconstructed right column. At tile row 0 the top edge
    // is the tile-top boundary (haveA = false), so ext_above / ext_corner are
    // not consulted by any predictor and are driven 0 (the corner for a
    // left-column py>0 leaf comes from ext_left[py-1], handled inside the recon
    // walk).
    recon1.input('ext_above').srcConnection! <= Const(0, width: f * 8);
    recon1.input('ext_left').srcConnection! <= sb0Right;
    recon1.input('ext_corner').srcConnection! <= Const(0, width: 8);

    output('frame1') <= recon1.output('frame');

    // sequencing FSM: walk SB0 -> latch -> walk SB1 (cont + cont_left) ->
    // latch -> recon SB0 -> wait -> recon SB1 (needs frame0) -> wait -> done.
    const sIdle = 0,
        sRunMode0 = 1,
        sLatch0 = 2,
        sRunMode1 = 3,
    ...
    Combinational([
      ...
      recon1Start < Const(0),
      Case(st, [
        // SB0: fresh start (cont = 0, above_open = 0, cont_left = 0,
        // left_open = 0).
        CaseItem(Const(sIdle, width: 4), [
          If(input('start'), then: [walkStart < Const(1)]),
        ]),
        // SB1: continue the window (cont = 1) at the tile-top (above_open = 0),
        // preserving the left context (cont_left = 1) with a SB to its left
        // (left_open = 1).
      ...
      If(
        ...
        orElse: [
          Case(st, [
            ...
            CaseItem(Const(sLatch0, width: 4), [
              ...HarborKeyframeDecodeMultiSbHoriz.latchConds(cap0, walk),
            ...
            CaseItem(Const(sLatch1, width: 4), [
              ...HarborKeyframeDecodeMultiSbHoriz.latchConds(cap1, walk),
  ...
  /// Latch the walk's leaf-array outputs into `cap`.
