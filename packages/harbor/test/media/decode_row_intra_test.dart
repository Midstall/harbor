...
// Reference: decode `blocks` 4x4 blocks from one stream and reconstruct a row.
List<List<int>> _refRow(List<int> buf, int blocks, int dcQ, int acQ) {
  final ec = _OdEc(buf);
  ...
  final frame = [for (var r = 0; r < 4; r++) List.filled(blocks * 4, 0)];
  var left = [128, 128, 128, 128];

  for (var gc = 0; gc < blocks; gc++) {
    final skip = cSkip.decode(ec);
    final y = cY.decode(ec);
    cUv.decode(ec); // uv (consumed, unused)
    ...
    final deq = [for (var i = 0; i < 16; i++) coeffs[i] * (i == 0 ? dcQ : acQ)];
    final residual = _invDct4(deq);
    final mode = y & 7;
    final firstCol = gc == 0;
    final lc = firstCol ? [128, 128, 128, 128] : left;

    int pred(int r, int c) {
      const above = 128, aboveRight = 128;
      final belowLeft = lc[3];
      final wv = _smW4[r], wh = _smW4[c];
      final vsum = wv * above + (256 - wv) * belowLeft;
      final hsum = wh * lc[r] + (256 - wh) * aboveRight;
      switch (mode) {
        case 1:
          return above; // vertical (above all 128)
        case 2:
        case 3:
          return lc[r]; // horizontal / paeth (above=al=128)
        case 4:
        ...
        case 6:
          return (hsum + 128) >> 8;
        case 7:
          return above; // d45 (above all 128)
        default: // dc
          return (512 + lc.reduce((a, b) => a + b) + 4) >> 3;
      }
    }
    ...
    for (var r = 0; r < 4; r++) {
      for (var c = 0; c < 4; c++) {
        recon[r][c] = _clamp(pred(r, c) + residual[r * 4 + c]);
        frame[r][gc * 4 + c] = recon[r][c];
      }
    }
    left = [for (var r = 0; r < 4; r++) recon[r][3]];
  }
  return frame;
}
...
void main() {
  ...
  group('HarborDecodeRowIntra (bitstream -> pixels)', () {
    for (final seed in [29, 91, 150, 222]) {
      test('decodes + reconstructs a 4-block row (seed $seed)', () async {
        const blocks = 4, dcQ = 16, acQ = 12;
        final buf = [for (var i = 0; i < 96; i++) (i * 37 + seed) & 0xFF];
        final exp = _refRow(buf, blocks, dcQ, acQ);

        final m = HarborDecodeRowIntra(blocks: blocks);
        ...
        for (var r = 0; r < 4; r++) {
          for (var x = 0; x < fw; x++) {
            final got = ((v >> ((r * fw + x) * 8)) & BigInt.from(0xFF)).toInt();
            expect(got, equals(exp[r][x]), reason: 'seed $seed ($r,$x)');
          }
        }
