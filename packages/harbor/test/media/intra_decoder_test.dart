...
int _pred(int mode, int r, int c, List<int> above8, List<int> left4, int al) {
  ...
  switch (mode) {
    ...
    default:
      return (above8.take(4).reduce((a, b) => a + b) +
...
// --- deblock ---
List<int> _filter4(List<int> px, int bl, int lim, int th) {
  final p3 = px[0], p2 = px[1], p1 = px[2], p0 = px[3];
  final q0 = px[4], q1 = px[5], q2 = px[6], q3 = px[7];
  var mask = true;
  if (_absd(p3, p2) > lim) mask = false;
  if (_absd(p2, p1) > lim) mask = false;
  if (_absd(p1, p0) > lim) mask = false;
  if (_absd(q1, q0) > lim) mask = false;
  if (_absd(q2, q1) > lim) mask = false;
  if (_absd(q3, q2) > lim) mask = false;
  if (_absd(p0, q0) * 2 + (_absd(p1, q1) >> 1) > bl) mask = false;
  if (!mask) return [p1, p0, q0, q1];
  final hev = _absd(p1, p0) > th || _absd(q1, q0) > th;
  ...
  final oq0 = _clampS8(qs0 - f1) + 128;
  final op0 = _clampS8(ps0 + f2) + 128;
  final f = hev ? 0 : (f1 + 1) >> 1;
  return [_clampS8(ps1 + f) + 128, op0, oq0, _clampS8(qs1 - f) + 128];
}

(int, int, int) _limits(int lvl, int sharp) {
  var bil = sharp != 0 ? lvl >> (1 + (sharp > 4 ? 1 : 0)) : lvl;
  final cap = 9 - sharp;
  if (bil > cap) bil = cap;
  if (bil < 1) bil = 1;
...
List<List<int>> _deblock(
  ...
  final g = [
    for (final row in f) [...row],
  ];
  for (var x = 4; x < w; x += 4) {
    for (var y = 0; y < h; y++) {
      final r = _filter4(
        [for (var k = -4; k < 4; k++) g[y][x + k]],
        bl,
        lim,
        th,
      );
...
void main() {
  ...
  group('HarborIntraDecoder (bitstream -> decoded + deblocked picture)', () {
    for (final seed in [29, 130, 201]) {
      test('decodes + deblocks a 3x3 frame (seed $seed)', () async {
        const gridW = 3, gridH = 3, dcQ = 16, acQ = 12, flvl = 20, sharp = 0;
        final buf = [for (var i = 0; i < 128; i++) (i * 53 + seed) & 0xFF];
        final dec = _refFrame(buf, gridW, gridH, dcQ, acQ);
        final (bl, lim, th) = _limits(flvl, sharp);
        final exp = _deblock(dec, gridW * 4, gridH * 4, bl, lim, th);

        final m = HarborIntraDecoder(gridW: gridW, gridH: gridH);
        ...
        for (final spec in [
          ...
          sig[spec.$1] = l;
          m.input(spec.$1).srcConnection! <= l;
        }
        ...
        sig['reset']!.inject(1);
        sig['start']!.inject(0);
        sig['bytes_in']!.inject(feed());
        sig['dc_q']!.inject(dcQ);
        sig['ac_q']!.inject(acQ);
        sig['filter_level']!.inject(flvl);
        sig['sharpness']!.inject(sharp);
        ...
        final fw = gridW * 4, fh = gridH * 4;
        // First confirm the undeblocked (reconstructed) frame matches decode.
        final rv = m.output('recon_frame').value.toBigInt();
        for (var y = 0; y < fh; y++) {
          for (var x = 0; x < fw; x++) {
            final got = ((rv >> ((y * fw + x) * 8)) & BigInt.from(0xFF))
                .toInt();
            expect(got, equals(dec[y][x]), reason: 'recon seed $seed ($y,$x)');
          }
        }
